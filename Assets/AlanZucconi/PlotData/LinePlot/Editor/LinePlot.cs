...
namespace AlanZucconi.Data
{
    public class LinePlot
    {
        ...
        private float GetY(float y)
        {
            ...
            //return Height - Mathf.InverseLerp(Data.Min.y, Data.Max.y, y) * Height;
        }
        ...
        public void OnGUI(Rect rect)
        {
            ...
            for (float y = 0; y <= Data.Max.y; y += Data.Max.y / Attribute.GridY)
                HorizontalLine(y);
            ...
            */
            ...
            // --- Bars -----------------
            if (Data.BarsData != null &&
                Data.BarsData.Count > 1)
            {
                // TODO; fix the clip area!
                //GUI.BeginClip(new Rect(0,0, 10,1000));
                GL.Begin(GL.TRIANGLE_STRIP);
                GL.Color(Attribute.DataColour.xA(0.05f));

                float minY = Mathf.Min(Data.Min.y, 0f);

                for (int i = 0; i < Data.BarsData.Count; i++)
                {
                    float x = GetX(Data.Data[i].x);

                    //Vector2 barMin = new Vector2(x, GetY(Data.BarsData[i].x));
                    //Vector2 barMax = new Vector2(x, GetY(Data.BarsData[i].y));

                    Vector2 barData = Data.BarsData[i];
                    Vector2 barMax = new Vector2(x, GetY(Mathf.Clamp(barData.y, minY, Data.Max.y)));
                    Vector2 barMin = new Vector2(x, GetY(Mathf.Clamp(barData.x, minY, Data.Max.y)));

                    //Vector2 barMax = new Vector2(x, Mathf.Min(GetY(Data.BarsData[i].y), Data.Max.y));
                    //Vector2 barMin = new Vector2(x, Mathf.Max(GetY(Data.BarsData[i].x), Data.Min.y));

                    GL.Vertex(barMax); // Max
                    GL.Vertex(barMin); // Min
                }

...
public static class Utils
{
    ...
    // Helper function to count the number of 1s in a long (number of bits set to 1)
    ...
    public static string FormatTime(float seconds)
    {
        ...
        if (span.Hours > 0 || span.Days > 0)  // Show hours if non-zero or if days are shown
            sb.Append($"{span.Hours}h ");

        if (span.Minutes > 0 || span.Hours > 0 || span.Days > 0)  // Show minutes if non-zero or if higher units are shown
